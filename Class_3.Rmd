...
", header = TRUE)
barplot(as.matrix(data2),main=" COVID 19 test Vs Gender",beside=TRUE,col=c("blue","purple"))
...
barplot(as.matrix(data2),main="Confirmed cases Vs Age categories",beside=TRUE,col=c("green","blue","purple"))
legend("topleft", c("Less than 15 years","15-49 years","Above 50"),cex= 1.0,bty="n",fill = c("green","blue","purple"))
```
```{r}
months<-c("Sept_2022","Oct_2022","Nov_2022","Dec_2022", "Jan_2023","Feb
_
2023","Mar_2023","Apr_2023","May_2023","June_2023")
...
geom_point(size=3)+ facet_grid(vs~am)+
...
geom_bar() geom_boxplot() geom_density() geom_histogram() geom_hline() geom_jitter() geom_line() #geom_point() geom_rug() #geom_smooth() geom_text() geom_violin() geom_vline() 
...
```
```{r}
hist(data_class$Height_cm, col = 'gold')
```

```{r}
hist(data_class$Weight_kg, col = "blue",main = "Sample Histogram")
```
...
## Data management with dyplr
Data source from the package nycflights13 and ggplot2 This data contains all
336,776 flights that departed from New york city.
```{r}
