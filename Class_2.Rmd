...
while: execute a loop while a condition is true • repeat: execute an infinite loop (must break out of it to stop) • break: break the execution of a loop • next: skip an interaction of a loop
if (condition){ ## do something } ## continue with the rest of the code. if( condition){ do something
} else { do something else
}
You can also create a series of test by following the initial if with a number of else ifs if(condition){ do something
...
summ_stat <- function(df){
  ...
  med <- median(df)
  # 1st quantile
  quant1<- quantile(df,0.25)
  #Third quantile
  quant3 <- quantile(df,0.75)
  #max
  Maximum <- max(df)
  #min
  Minimun <- min(df)
  return(list(Sum = total,Mean = mn, First_quantile = quant1,Median= med,
              Third_Quantile = quant3, Maximum = Maximum, Minimum = Minimun ))
  
}
data('mtcars')
#calling the function 
summ_stat(mtcars$mpg)
```
```{r}
#Ex2
ttest <- function(df1,df2, conf.level = 0.95){
  #t test 
  test_result <- t.test(df1,df2, 
                        conf.level = conf.level)
  return(test_result)
}
#Calling the function
ttest(mtcars$mpg,mtcars$wt)
```
...
```
It works for any number of columns.There is no way to accidentally miss a columnThere is no way to accidentally treat one column differently than another
...
Another important functions when dealing with big data is split() -tapply
## Titanic data
```{r}
str(Titanic)
```
...
```
Grades the students: quantile function gives the percentile rank o f each student's performance score check the cutoff of A
```{r}
